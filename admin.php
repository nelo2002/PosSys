...
if(isset($_POST['login']))
{
	...
	$sql="SELECT * FROM `shops` WHERE username='$username'";
	$result=mysqli_query($conn, $sql);

	if($result->num_rows >0 )
	{
		$sql2="SELECT * FROM `shops` WHERE username='$username' AND password='$passwprd'";
		$result2=mysqli_query($conn,$sql2);
		if($result2->num_rows > 0)
		{
			$user = mysqli_fetch_assoc($result);

			$exdate=strtotime($user['date']);
			$today=strtotime(date("d-m-Y"));

			if($exdate<$today)
			{
				$sql ="UPDATE `shops` SET `status`='deactivate' WHERE  id='$user[id]'";
 				$result=$conn->query($sql);

			}
			else{
				$sql ="UPDATE `shops` SET `status`='Active' WHERE  id='$user[id]'";
 				$result=$conn->query($sql);
			}
			$result3=mysqli_query($conn,$sql2);
			$user1 = mysqli_fetch_assoc($result3);
	...
	<div class="limiter">
		<div class="container-login100">
			<div class="wrap-login100">
				...
				<form action="admin.php" class="form1 login100-form validate-form" id="form1" onsubmit="return validate()" method="post" >
					...
					<div class="text-center p-t-12">
						...
						<a class="txt2" href="otp_email.php" style="text-decoration: none;">
