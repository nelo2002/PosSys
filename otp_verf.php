...
if(isset($_POST['check'])){
        ...
        if(mysqli_num_rows($code_res) > 0){
            ...
            $code = 0;
            $status = 'verified';
            $sql="UPDATE  `staff` SET `is_active`='$status',`code`='$code' WHERE code='$fetch_code'";
            if ($conn->query($sql) === TRUE) {
				...
				echo"<script>Swal.fire({
					title: 'OTP Verification',
					icon: 'error',
					text: 'your account has been not activated',
					confirmButtonText: 'OK',
				   
				  }).then((result) => {
					if (result.isConfirmed) {
					  window.location.assign('otp_verf.php')
					} 
				  })</script>";
              }
    ...
    ?>
		...
		<form action="otp_verf.php" method="post">
                <div id="app">
                    <div class="container height-100 d-flex justify-content-center align-items-center">
                        <div class="position-relative">
                            <div class="card p-2 text-center">
                                ...
                                <div class="mt-4"> <button class="btn btn-danger px-4 validate" name="check" type="submit">Validate</button> </div>
								
								<div class="mt-3 content d-flex justify-content-center align-items-center"> <span>Didn't get the code ?<a href="resendotp.php?id='send'" class="text-decoration-none ms-3"><b>Resend</b></a></span> </div>
                            </div>
                        </div>
                    </div>
                </div>
